...
// --- 数値関係 ---
int default_intvl_us = 10000; // デフォルトのパルス幅
int pulse_width = 10000; // 現在のパルス幅
int step_count = 0; // 現在のステップ数（座標）
int MAX_PULSEWIDTH = 10000;
float z_n = 0; // 現在の取得座標
float z_n_1 = 0; // 前の取得座標
int t_n_1 = 0;
float z_origin = 0; // アクチュエータが原点にいるときのz座標
// ----------------------
...
void loop() {
  ...
  if (Serial.available() > 0)
  {
    ...
    if (ExecutionStart) {
      z_n = str.toFloat();
      z_n_1 = z_n;
      z_origin = z_n; // アクチュエータの原点時のUnity内のz座標
      t_n_1 = t_n;
    ...
    // パルス幅計算
    z_n_1 = z_n;
    int delta_t_n = t_n - t_n_1;
    z_n = str.toFloat();
    float delta_z_n = z_n - z_n_1;
    // float step = 4000 * delta_z_n;
    int s_x_n = step_count;
    int s_x_nplus1 = (((z_n - z_origin)*1000.0) / 6.0) * 100.0;
    int delta_s = s_x_nplus1 - s_x_n;

    if (delta_s == 0) pulse_width = MAX_PULSEWIDTH;
    else pulse_width = delta_t_n / delta_s;
    // pulse_width = (int)(delta_t_n / step);
    // pulse_width = 100;
    t_n_1 = t_n;
    ...
    // Serial.write(delta_s);
    // Serial.print("delta_t = ");
    // Serial.println(delta_t_n);
    //Serial.println(delta_z_n, 10);
    Serial.print("pulse = ");
  ...
  DoesStop = (abs(pulse_width) >= MAX_PULSEWIDTH);
